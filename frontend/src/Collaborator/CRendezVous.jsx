...
export default function RendezVousCollaborator() {
  ...
  const handleStatusChange = async (id, action) => {
    try {
      ...
      alert(`Rendez-vous ${action === "confirm" ? "confirmé" : "annulé"} !`);
            ...
            {appointments.map((a) => (
              <tr key={a.id} className="border-b hover:bg-blue-50 transition-colors">
                <td className="py-3 px-4 whitespace-nowrap font-medium text-gray-700">
                  {a.patient?.user?.profile?.first_name} {a.patient?.user?.profile?.last_name}
                </td>
                <td className="py-3 px-4 whitespace-nowrap">{new Date(a.date).toLocaleDateString()}</td>
                <td className="py-3 px-4 whitespace-nowrap">{a.time || "-"}</td>
                <td className={`py-3 px-4 font-semibold capitalize ${
                  a.status === 'confirmed' ? 'text-green-600' :
                  a.status === 'canceled' ? 'text-red-600' : 'text-gray-600'
                }`}>{a.status}</td>
      ...
      <div className="md:hidden space-y-4">
        {appointments.map((a) => (
          <div key={a.id} className="bg-white p-4 rounded-xl shadow hover:shadow-md transition">
            ...
            <p className="text-sm text-gray-500">Date: {new Date(a.date).toLocaleDateString()}</p>
            <p className="text-sm text-gray-500">Heure: {a.time || "-"}</p>
            ...
            {a.telehealthLink && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-500 underline text-sm hover:text-blue-700 transition-colors"
              >
